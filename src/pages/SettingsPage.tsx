...
export function SettingsPage() {
  ...
  const reportRawPreviewUrl = `http://127.0.0.1:${reportPreviewPort}/report?token=${reportPreviewToken}`;
  ...
  const handleSaveNetworkConfig = async () => {
    ...
    try {
      ...
      if (reportEnabled && normalizedToken === 'change-this-token') {
        alert('WebReport token 不能使用默认值，请先生成新的 token');
        return;
      }
                    ...
                    <div className="settings-row" style={{ animation: 'fadeUp 0.3s ease both' }}>
                      ...
                      <div className="row-control">
                        <span style={{
                          ...
                          http://127.0.0.1:{reportActualPort}
                        </span>
                      </div>
                    </div>
                  )}
